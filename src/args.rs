...
pub(crate) fn parse_startup_args() -> Result<LdkUserInfo, ()> {
	// Split off `--`-style flags so the positional arguments below keep their indices.
	let mut pq_listen_port: Option<u16> = None;
	...
	for arg in env::args() {
		if let Some(port) = arg.strip_prefix("--pq-listen-port=") {
			pq_listen_port = Some(port.parse::<u16>().expect("Failed to parse --pq-listen-port"));
		} else if arg == "--pq-require-payments" {
			pq_require_payments = true;
		} else if arg == "--pq-require-inbound" {
	...
	if pq_listen_port.is_some() || pq_require_payments || pq_require_inbound || pq_blinded_paths {
		println!("ERROR: --pq-* flags require a build with the post-quantum feature");
		return Err(());
	}
	...
	let bitcoind_rpc_info = args[1].clone();
	...
	let ldk_storage_dir_path = args[2].clone();

	let mut ldk_peer_port_set = true;
	let ldk_peer_listening_port: u16 = match args.get(3).map(|p| p.parse()) {
		Some(Ok(p)) => p,
		Some(Err(_)) => {
	...
	let network: Network = match args.get(arg_idx).map(String::as_str) {
	...
	let ldk_announced_node_name = match args.get(arg_idx + 1) {
		Some(s) => {
			if s.len() > 32 {
